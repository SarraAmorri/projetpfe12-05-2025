...
export default function AdminLocation() {
  ...
  const handleDecision = async (id, decision, montantValue) => {
    try {
      ...
      const response = await axios.put(
        ...
        { decision, montant: decision === 'acceptee' ? montantValue : 0 },
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );

      setDemandes((prev) =>
        prev.map((demande) =>
          demande._id === id ? { ...demande, etat: decision, montant: response.data.montant } : demande
        )
      );
    } catch (err) {
  ...
  useEffect(() => {
    const fetchDemandes = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/locationbus/all`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          {demandes.map((demande) => (
            <div key={demande._id} className="bg-white p-6 rounded-xl shadow-md space-y-2">
              <p><strong>De :</strong> {demande.de}</p>
              <p><strong>À :</strong> {demande.a}</p>
              <p><strong>Date :</strong> {new Date(demande.date).toLocaleDateString()}</p>
              <p><strong>Durée :</strong> {demande.duree} jours</p>
              <p><strong>Type de bus :</strong> {demande.typeBus}</p>
              <p><strong>Client :</strong> {demande.clientEmail}</p>
              <p><strong>État :</strong> {demande.etat}</p>
              {demande.etat === 'acceptee' && <p><strong>Montant :</strong> {demande.montant} DT</p>}
              {demande.etat === 'en_attente' && (
                <div className="mt-4">
                  ...
                  <input
                    ...
                    onChange={(e) => handleMontantChange(demande._id, e.target.value)}
                  ...
                  <div className="flex gap-4">
                    <button
                      onClick={() => handleDecision(demande._id, 'acceptee', montant[demande._id])}
